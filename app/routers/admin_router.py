...
def get_users(current_user: User = Depends(get_current_active_user), db: Session = Depends(get_database)):
    """
    Получение списка всех пользователей. Только администратор имеет доступ.
    """
...
def update_user(user_id: int, role: RolesEnum,
                current_user: User = Depends(get_current_active_user),
                db: Session = Depends(get_database)):
    """
    Обновление роли пользователя. Только администратор имеет доступ.
    """
    if current_user.role != RolesEnum.ADMIN:
        raise HTTPException(status_code=403, detail="Not enough permissions")
...
def delete_user(user_id: int,
                ...
                db: Session = Depends(get_database)):
    """
    Удаление пользователя. Только администратор имеет доступ.
    """
    if current_user.role != RolesEnum.ADMIN:
        raise HTTPException(status_code=403, detail="Not enough permissions")
    user = db.query(User).filter(User.id == user_id).first()
